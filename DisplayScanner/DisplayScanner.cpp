...
std::wstring getMonitorName(HMONITOR monitor) {
    ...
    for (auto& p : paths) {
        ...
        if (wcscmp(info.szDevice, sourceName.viewGdiDeviceName) == 0) {
            ...
            //name.edidManufactureId = p.sourceInfo.id

            //std::cout << ";ID Adattatore Low:" << name.header.adapterId.LowPart << ";RefreshRate:" << p.targetInfo.refreshRate.Numerator << ";ID Monitor:" << p.targetInfo.id << ";";


            std::cout << ";" << name.header.adapterId.LowPart << ";" << p.targetInfo.refreshRate.Numerator << ";" << p.targetInfo.id << ";";




            //
            //std::cout << "IDMonitor:" << name.header.id << "\n\n";
...
)
{
    ...
    wprintf(L"%s", mi.szDevice);

    std::wcout << L"" << getMonitorName(hMonitor) << L"\n";
    return TRUE;
}
